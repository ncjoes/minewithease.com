@extends('layouts.core.admin')

@section('content')
    <div class="page-header">
        <h3>
            Manage Payment Channels
            <small>(Match: {{$result_count}}; Grand Total: {{$net_count}})</small>
            <a href="{{route('core.admin.channel.create')}}" class="btn btn-success pull-right">New...</a>
        </h3>
    </div>

    <div class="panel panel-default">
        <div class="panel-heading">
            <form action="{{route('core.admin.channel.manage')}}" method="GET"
                  enctype="application/x-www-form-urlencoded">
                <div class="row">
                    <div class="col-sm-4">
                        <div class="form-group">
                            <label for="search" class="small">Channel Name</label>
                            <input name="search" id="search" class="form-control" value="{{$filter['search']}}">
                        </div>
                    </div>

                    <div class="col-sm-2">
                        <div class="form-group">
                            <label for="currency" class="small">Currency</label>
                            <select name="currency" id="currency" class="form-control">
                                <option></option>
                                @foreach($currencies as $currency)
                                    <option value="{{$currency->alpha_code}}" @if($filter['currency']==$currency->alpha_code) selected @endif>{{$currency->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="col-sm-2">
                        <div class="form-group">
                            <label for="status" class="small">Active?</label>
                            <select name="status" id="status" class="form-control">
                                @foreach($statuses as $key=>$label)
                                    <option value="{{$key}}" @if($filter['status']==$key) selected @endif>{{$label}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="col-sm-2 d-none d-sm-inline-block">
                        <div class="form-group">
                            <label for="per_page" class="small">Rows Per Page</label>
                            <input name="per_page" id="per_page" type="number" value="{{$filter['per_page']}}" class="form-control">
                        </div>
                    </div>
                    <div class="col-sm-2">
                        <label></label>
                        <button type="submit" class="btn btn-primary form-control btn-block btn-sm"><i class="fa fa-search">Search</i></button>
                    </div>
                </div>
            </form>
        </div>
        <form class="ajax-form" method="post" action="{{route('core.admin.channel.manage')}}">
            {{csrf_field()}}
            <div class="panel-body">
                <div class="table-responsive">
                    <table class="table table-bordered table-hover" id="basic-datatable">
                        <thead>
                        <tr>
                            <th width="4em">##</th>
                            <th>Local Currency</th>
                            <th>Name</th>
                            <th class="money">Min. Amount</th>
                            <th class="money">Max. Amount</th>
                            <th>Is Active?</th>
                            <th>For Inflow?</th>
                            <th>For Outflow?</th>
                            <th>&hellip;</th>
                            <th width="3em"><i class="fa fa-check-square"></i></th>
                        </tr>
                        </thead>
                        <tbody>
                        @foreach($results as $_result)
                            <tr>
                                <td>{{$loop->index+1}}</td>
                                <td><label for="i-{{$_result->id}}">{{$_result->currency->name}}</label></td>
                                <td><label for="i-{{$_result->id}}"><img src="{{$_result->photo_url}}" alt="icon" style="max-width: 1.5em;"> {{$_result->name}}</label></td>
                                <td class="money">{{$_result->minAmountStr()}}</td>
                                <td class="money">{{$_result->maxAmountStr()}}</td>
                                <td>{{$_result->status()}}</td>
                                <td>{{$_result->inflow()}}</td>
                                <td>{{$_result->outflow()}}</td>
                                <td class="center"><a href="{{$_result->edit_url}}"><i class="fa fa-pencil"></i>Details</a></td>
                                <td class="center"><input type="checkbox" name="ids[]" value="{{$_result->id}}" id="i-{{$_result->id}}"></td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
                </div>
            </div>
            <div class="panel-footer clearfix">
                <div class="pull-left">{{$results->appends($filter)->links()}}</div>
                <div class="pull-right">
                    <select class="form-control text-left d-inline btn btn-sm" style="width: auto;" name="action">
                        <option value="activate">Activate</option>
                        <option value="deactivate">De-Activate</option>
                        <option value="delete">Delete Forever</option>
                    </select>
                    <button type="submit" class="btn btn-sm btn-primary">Go</button>
                </div>
            </div>
        </form>
    </div>
@endsection
