...
@section('content')
    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <form action="{{route('core.admin.country.manage')}}" method="GET"
        ...
        <div class="panel-body">
            <form class="ajax-form" method="post" action="{{route('core.admin.country.manage')}}">
                {{csrf_field()}}
                    ...
                    <table class="table table-bordered table-hover" id="basic-datatable">
                        ...
                        @foreach($results as $_result)
                            <tr>
                                ...
                                <td>{{$_result->continent->name}}</td>
                                <th>{{$_result->iso2}}</th>
                                <th>{{$_result->iso3}}</th>
                                <td>{{$_result->regions()->count()}}</td>
                                <td>{{$_result->status()}}</td>
                                <td class="center"><a href="{{$_result->url}}"><i class="fa fa-eye"></i> Details</a></td>
